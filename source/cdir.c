...
/*
	...
	arguments: 
		1. path to the new directory:
			1.1 relative from the current directory
			1.2 absolute path
*/
void cdir(char* parsed[],int count){
	if(count == 1){
		fprintf(stderr, "kfsh: No folder to change to\n");
		return;
	}
	...
	if(!ret_val){
		...
		switch(errno){
			...
			case ENOTDIR: {
				fprintf(stderr, "kfsh: Component of path is not a directory\n");
				break;
			}
			case ENAMETOOLONG: {
			...
			default:{
				fprintf(stderr, "kfsh: Could not change directory\n");
				break;
			}
		}
	}
